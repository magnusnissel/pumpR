...
p <- p + scale_fill_gradient2(low='white', high='steelblue')
p

sets_months <- log_df %>% group_by(MONTH) %>% summarise(num_sets =  n())
p <- ggplot(sets_months, aes(x = MONTH, y=num_sets, fill=num_sets)) + geom_bar(stat = "identity") 
...
As much as isolation exercises help for hypertrophy, there is a good reason that four compound lifts have a special status ^[Feel free to argue that the Power Clean or Barbell Row should make the list, I'm siding with history on this one.].
...
Below are some stats on these four lifts, both in the _strict_ sense (counting only the classic barbell versions) and including all kinds of _variants_ from _machine hack squats_ to _one-armed presses_ with _extra thick dumbbells_.
